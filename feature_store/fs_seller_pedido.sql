...
tb_summary(
  select 
         ...
         count(distinct case when datediff('{date}',dtPurchase) < 30 then idOrder end) / (count(distinct t1.idOrder) / count(distinct date_format(dtPurchase, 'y_M'))) as qtRazaoPedidoVsMedia,
